...
client = OpenAI(
    api_key="ms-f8e33591-4d78-43eb-b9d6-af96883c3e2f",  # 请替换为你的 ModelScope Access Token
...
def summarize_chunk_with_ai(chunk_text, chunk_index, total_chunks):
    """使用 AI 对单个文本块进行总结"""
    prompt = f"""
...
def combine_summaries_with_ai(summaries):
    """将所有总结合并成最终文档"""
...
def summarize_large_markdown(file_path, output_path=None):
    ...
    for i, chunk in enumerate(chunks):
        ...
        summary = summarize_chunk_with_ai(chunk, i, len(chunks))
        summaries.append(summary)

        # 保存中间结果以防中断
        temp_filename = f"temp_summary_{i+1:03d}.md"
    ...
    # 合并所有总结
    print("正在生成最终总结报告...")
    final_summary = combine_summaries_with_ai(summaries)

    # 输出结果
    if output_path is None:
        output_path = "comprehensive_summary.md"
...
if __name__ == "__main__":
    # 处理读本.md文件
    input_file = "读本.md"  # 处理你提到的读本文档

    # 检查文件是否存在
    if not os.path.exists(input_file):
        print(f"警告：文件 {input_file} 不存在，请确认文件路径正确")
        # 列出当前目录的文件供参考
        print("当前目录文件:")
        ...
        result = summarize_large_markdown(input_file, "comprehensive_summary.md")
        if result:
            print("总结已完成！")
            print("生成的总结报告已保存为 comprehensive_summary.md")